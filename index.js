...
client.on("messageCreate", async (ctx) => {
  ...
  if (cmd === "test") {
    ...
    ctx.channel.send({
      ...
      embeds: [{
        color: "2F3136",
        title: `${emoji.yes} The bot is functional`
  ...
  if (cmd === "botinfo") {
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setColor('RANDOM')
      .setURL('https://discord.gg/tQVAMSkWqG')
      .setThumbnail(client.user.displayAvatarURL({ dynamic: true }))


      .addFields(
        { name: "🍭・Information", value: `> **Bot: :** <@${client.user.id}> \`\`${client.user.username}\`\`\n> **ID :** ${client.user.id}\n>  \`\`\`\``, inline: false },
        { name: "🍭Developer", value: `> **Name :** .tellygamerz`, inline: false },
      )
    ctx.channel.send({
      embeds: [embed]
    })
  ...
  if (cmd === "partner") {
    ...
    ctx.channel.send({
      embeds: [{
        ...
        description: `> **No one is partner dm  to become a parther.**`,
        color: "2F3136",
        footer: {
          "text": `${kalash.client} ・ ${kalash.footer}`,
          "icon_url": `https://cdn.discordapp.com/attachments/785104663544463390/880023050941255680/774322042970832926.gif`
  ...
  if (cmd === "verify") {
    ...
    ctx.channel.send({
      ...
      embeds: [{
        ...
        description: `**To be able to see the private giveaways channels and chat channels verify now click\n [here!](${kalash.authLink}) ✅ **`,
        "color": 16711680,
